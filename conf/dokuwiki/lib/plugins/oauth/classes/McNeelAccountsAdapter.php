...
use OAuth\OAuth2\Service\McNeelAccounts;
 ...
 *  
 *  AJ: This file has been modified to work with McNeel Accounts.
 *
 * @link https://github.com/doorkeeper-gem/doorkeeper
 * @package OAuth\Plugin
 */
class McNeelAccountsAdapter extends AbstractAdapter {
    ...
    public function getUser() {
        ...
        $result = $JSON->decode($this->oAuth->request($this->hlp->getUserInfoEndpoint('McNeelAccounts')));
    ...
    public function getScope() {
        return array(McNeelAccounts::SCOPE_EMAIL, McNeelAccounts::SCOPE_PROFILE);
    }

    public function login() {
        $parameters = array();
        $parameters['state'] = urlencode(base64_encode(md5(rand())));
        $this->storage->storeAuthorizationState('McNeelAccounts', $parameters['state']);
